...
class DetectSceneHelper:
    ...
    def get_match(
        ...
        if script_logger.get_log_level() == 'info':
            matching_overlay_relative_path = script_logger.get_log_path_prefix() + 'detectScene-matchOverlayed.png'
            cv2.imwrite(matching_overlay_relative_path, screencap_im_bgr)
            script_logger.get_action_log().add_supporting_file_reference('image', matching_overlay_relative_path)
            masked_img_relative_path = script_logger.get_log_path_prefix() + 'detectScene-maskApplied.png'
            cv2.imwrite(masked_img_relative_path, screencap_masked)
            script_logger.get_action_log().add_supporting_file_reference('image', masked_img_relative_path)
            comparison_img_relative_path = script_logger.get_log_path_prefix() + 'detectScene-comparisonImage.png'
            cv2.imwrite(comparison_img_relative_path, screencap_compare)
